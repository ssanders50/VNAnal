...
int ep2a, int ep2b, int ep2c,int ep3a, int ep3b, int ep3c, int mintrk, int maxtrk,int subevt);
  void flip();
  void add(TH2D * qxtrk, TH2D * qytrk, TH2D * qcnt, double * qx, double* qy, double * qw, TH2D * avpt, int noff);
  TH2D * getRescor();
  TH1D * getRescorAv();
  TH2D * getEPVN();
  TH2D * getEPVNobs();
...
int ep3a, int ep3b, int ep3c, int mintrk, int maxtrk, int subevt) {
  ...
  std::string utag = Form("_%s_%s_%s_%s_%s_%s_%s_%d_%d_%d",info->getFileName().Data(),
  ...
  string qavn = "qav_"+utag;
  qav = new Qvec(0,0,qavn);
  for(int i = 0; i<ptbins->GetNbinsX(); i++) {
    for(int j = 0; j<etabins->GetNbinsX(); j++) {
      string name = Form("q_%d_%d_",i,j);
      name+=utag;
...
void GenSupport::add(TH2D * qxtrk, TH2D * qytrk, TH2D * qcnt, double * qx, double * qy, double * qw, TH2D *  avpt, int evtchar){
  ++count;
  ep2a_ = ep2aSet_;
  ep2b_ = ep2bSet_;
  ep2c_ = ep2cSet_;
  ...
  for(int i = 0; i<ptbins->GetNbinsX(); i++) {
    ...
    for(int j = 0; j<etabins->GetNbinsX(); j++) {
      ...
      comp qn(qxtrk->GetBinContent(i+1,jeta), qytrk->GetBinContent(i+1,jeta));
