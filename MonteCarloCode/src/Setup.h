...
void OutputResults(TString reac, TString midn, TString trig){
  ...
  if(trig.Contains("data")) strig="MB";
  ...
  string recenter = "";
  if(RECENTERTRACKS) recenter="_recenter";
  ...
  for(int i = 0; i<nbins; i++) {
	...
	if(genp[i][j]->gethavab()) genp[i][j]->gethavab()->Write("QAB.real");
	if(j==0) {
  ...
  for(int i = 0; i<nbins; i++) {
	...
	if(genm[i][j]->gethavab()) genm[i][j]->gethavab()->Write("QAB.real");
	if(j==0) {
	  if(qxav[i]) qxav[i]->Write("qxav");
	  if(qyav[i]) qyav[i]->Write("qyav");
	  if(qxavchk[i]) qxavchk[i]->Write("qxavchk");
